...
10.times do
  ...
  internship = Internship.new(title: "Awesome Developer#{n}", salary: r_salary, internship_rating_id: iR.id, working_hours: r_work,
                              living_costs: r_living, company_id: company.id, student_id: student.id, semester_id: semester.id,
                              start_date: Time.at(rand*Time.now.to_f).to_date, due_date: Time.at(rand*Time.now.to_f).to_date, end_date: Time.at(rand*Time.now.to_f).to_date, operational_area: Orientation.offset(r_o).first,
...
report = Report.new(title: "Awesome Developer#{n}", company_id: company.id, student_id: student.id, semester_id: semester.id,
                        start_date: Time.at(rand*Time.now.to_f).to_date, due_date: Time.at(rand*Time.now.to_f).to_date, end_date: Time.at(rand*Time.now.to_f).to_date,
                         supervisor_name: "b"*r_b, supervisor_email: "e"*r_e, report_state_id: r_re_s, certificate_state_id: r_ce_s, internship_state_id: r_i_s, comment: "p"*r_p, reading_prof_id: reading_prof_id, certificate_to_prof: Time.at(rand*Time.now.to_f).to_date,
                        certificate_signed_by_prof: Time.at(rand*Time.now.to_f).to_date, certificate_signed_by_internship_officer: Time.at(rand*Time.now.to_f).to_date)
...
User.create!(password: "testmap2", email: "test2@imimaps.com", student_id: 2)
User.create!(password: "testmap3", email: "test3@imimaps.com", student_id: 3)